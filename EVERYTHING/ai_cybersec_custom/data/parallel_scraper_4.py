#!/usr/bin/env python3
"""
PARALLEL SCRAPER #4 - Cloud Security & Vulnerability Tools
...
# SCRAPER 4: Cloud Security & Vuln Tools
REPOS = [
    "Azure/Azure-Security-Center", "aquasecurity/trivy", "bridgecrewio/checkov",
    "RhinoSecurityLabs/pacu", "nccgroup/ScoutSuite", "toniblyx/prowler",
    "projectdiscovery/nuclei-templates", "projectdiscovery/nuclei",
    "sqlmapproject/sqlmap", "commixproject/commix", "epinna/tplmap",
    "urbanadventurer/WhatWeb", "EnableSecurity/wafw00f", "xmendez/wfuzz",
    "OJ/gobuster", "ffuf/ffuf", "tomnomnom/meg",
]

def log(msg):
    print(f"[SCRAPER-4] [{datetime.now().strftime('%H:%M:%S')}] {msg}", flush=True)
...
def scrape_github_repo(owner_repo, max_files=1000):
    try:
        ...
        files = [f for f in tree if f['type'] == 'blob' and 
                 any(f['path'].endswith(ext) for ext in ['.md', '.txt', '.py', '.go', '.yaml', '.yml'])]
...
log("🚀 SCRAPER 4 STARTED: Cloud Security & Vuln Tools")
cycle = 1
files_per_repo = 600
...
while True:
    try:
        ...
        for i, repo in enumerate(REPOS, 1):
            ...
            if data:
                new = save_data(data, f"s4_{repo.replace('/', '_')}.jsonl")
                log(f"  ✅ +{new} items")
            time.sleep(2)
        
        total_size = sum(f.stat().st_size for f in OUTPUT_DIR.glob("s4_*.jsonl"))
