#!/usr/bin/env python3
"""
PARALLEL SCRAPER #3 - CTF & Reverse Engineering
...
REPOS = [
    "Gallopsled/pwntools", "pwndbg/pwndbg", "longld/peda", "hugsy/gef",
    "NationalSecurityAgency/ghidra", "radare/radare2", "rizinorg/rizin",
    "ctfs/write-ups-2016", "ctfs/write-ups-2015", "p4-team/ctf",
    "apsdehal/awesome-ctf", "facebook/fbctf", "CTFd/CTFd",
    "google/google-ctf", "RsaCtfTool/RsaCtfTool",
]

def log(msg):
    print(f"[SCRAPER-3] [{datetime.now().strftime('%H:%M:%S')}] {msg}", flush=True)
...
def scrape_github_repo(owner_repo, max_files=1000):
    try:
        ...
        files = [f for f in tree if f['type'] == 'blob' and 
                 any(f['path'].endswith(ext) for ext in ['.md', '.txt', '.py', '.c', '.cpp', '.asm'])]
...
log("🚀 SCRAPER 3 STARTED: CTF & Reverse Engineering")
cycle = 1
files_per_repo = 600
...
while True:
    try:
        ...
        for i, repo in enumerate(REPOS, 1):
            ...
            if data:
                new = save_data(data, f"s3_{repo.replace('/', '_')}.jsonl")
                log(f"  ✅ +{new} items")
            time.sleep(2)
        
        total_size = sum(f.stat().st_size for f in OUTPUT_DIR.glob("s3_*.jsonl"))
