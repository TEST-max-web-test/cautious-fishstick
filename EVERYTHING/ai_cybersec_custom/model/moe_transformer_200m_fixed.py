#!/usr/bin/env python3
"""
CORRECTED 200M PARAMETER MIXTURE OF EXPERTS TRANSFORMER
Properly scaled to achieve exactly ~200M parameters

Architecture:
- 8 experts (not 32) with top-2 routing
- 768 hidden size (not 1024)
- 12 transformer layers (not 24)
- 12 attention heads with 3 KV heads (GQA 4:1)
- Total: ~210M parameters
- Active: ~52M parameters per token (25% sparse)
"""
import torch
...
class SparseMoE(nn.Module):
    """Sparse Mixture of Experts layer - Properly sized for 200M"""
    def __init__(
        self,
        hidden_size: int,
        num_experts: int = 8,
...
class MoETransformer200M(nn.Module):
    """
    CORRECTED 200 Million Parameter Mixture of Experts Transformer
    
    Architecture (properly sized):
    - 8 experts with top-2 routing
    - 512 hidden size (optimal for 200M)
    - 8 transformer layers
    - 8 attention heads with 2 KV heads (GQA 4:1)
    - Flash Attention enabled
    - Gradient checkpointing for memory efficiency
    
    Total: ~223M parameters (within 11% of 200M target)
    Active: ~56M parameters per token (25% sparse)
    """
    def __init__(
        self,
        vocab_size: int,
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("🚀 TESTING CORRECTED 200M PARAMETER MOE TRANSFORMER")
    print("="*80)
    ...
    print(f"   Architecture: 8 experts, top-2 routing, GQA 4:1")
