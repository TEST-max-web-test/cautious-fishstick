#!/usr/bin/env python3
import os
import sys
import torch

# Ensure parent directories are on sys.path so ai_cybersec_custom resolves
script_dir = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, os.path.join(script_dir, '..'))
sys.path.insert(0, os.path.dirname(os.path.join(script_dir, '..')))

from ai_cybersec_custom.model.custom_transformer import CustomTransformer
from ai_cybersec_custom.tokenizer.custom_tokenizer import CustomTokenizer
from ai_cybersec_custom.utils.config import MODEL_CONFIG, INFER_CONFIG, TRAIN_CONFIG


def load_model_and_tokenizer(device: str = 'cpu'):
    """Load model and tokenizer from checkpoint."""
    # Tokenizer: try a list of plausible paths relative to script and package
    possible_tokenizer_paths = [
        os.path.join(script_dir, 'tokenizer', 'bpe.model'),
        os.path.join(script_dir, '..', 'tokenizer', 'bpe.model'),
        os.path.join(script_dir, '..', '..', 'tokenizer', 'bpe.model'),
        os.path.join(script_dir, '..', 'bpe.model'),
        'bpe.model',
        os.path.join('ai_cybersec_custom', 'tokenizer', 'bpe.model'),
    ]

    tokenizer_path = None
    for path in possible_tokenizer_paths:
        if os.path.exists(path):
            tokenizer_path = path
            break

    if not tokenizer_path:
        print("❌ ERROR: Could not find bpe.model in any of these locations:")
        for path in possible_tokenizer_paths:
            print(f"   - {os.path.abspath(path)}")
        raise FileNotFoundError("bpe.model not found!")

    print(f"📂 Loading tokenizer from: {tokenizer_path}")
    tokenizer = CustomTokenizer(tokenizer_path)

    # Instantiate model
    model = CustomTransformer(
        MODEL_CONFIG['vocab_size'],
        MODEL_CONFIG['hidden_size'],
        MODEL_CONFIG['num_layers'],
        MODEL_CONFIG['num_heads'],
        MODEL_CONFIG['ff_expansion']
    )

    # Explicitly include the workspace path requested plus other fallback locations
    workspace_checkpoint = os.path.join(
        os.path.sep,
        'workspaces',
        'cautious-fishstick',
        'ai_cybersec_custom',
        'train',
        'utils',
        'checkpoint.pt'
    )

    possible_checkpoint_paths = [
        # priority: TRAIN_CONFIG if provided and absolute
        TRAIN_CONFIG.get('checkpoint_path') if isinstance(TRAIN_CONFIG, dict) else None,
        workspace_checkpoint,
        os.path.join('ai_cybersec_custom', 'utils', 'checkpoint.pt'),
        os.path.join('train', 'utils', 'checkpoint.pt'),
        os.path.join(script_dir, 'utils', 'checkpoint.pt'),
        os.path.join(script_dir, '..', 'utils', 'checkpoint.pt'),
        'utils/checkpoint.pt',
        'checkpoint.pt',
    ]

    # Filter out None values and preserve order
    possible_checkpoint_paths = [p for p in possible_checkpoint_paths if p]

    checkpoint_path = None
    for path in possible_checkpoint_paths:
        if os.path.exists(path):
            checkpoint_path = path
            break

    if checkpoint_path and os.path.exists(checkpoint_path):
        # torch.load signature doesn't accept weights_only; use map_location only
        state = torch.load(checkpoint_path, map_location=device)
        # state may be a dict with 'model_state_dict' or plain state_dict
        if isinstance(state, dict) and ('model_state_dict' in state or 'state_dict' in state):
            sd = state.get('model_state_dict', state.get('state_dict', state))
        else:
            sd = state
        try:
            model.load_state_dict(sd, strict=False)
            print(f"✅ Loaded checkpoint from {checkpoint_path}")
        except Exception as e:
            print(f"⚠️  Failed to strictly load checkpoint: {e}")
            # attempt partial/state key adaptation if possible
            # try to extract nested "params" or "model" keys commonly used
            possible_sd = None
            for key in ('params', 'model', 'module', 'model_state'):
                if isinstance(state, dict) and key in state and isinstance(state[key], dict):
                    possible_sd = state[key]
                    break
            if possible_sd:
                model.load_state_dict(possible_sd, strict=False)
                print(f"✅ Loaded checkpoint (adapted) from {checkpoint_path}")
            else:
                print("⚠️  Could not adapt checkpoint automatically; continuing with random weights")
    else:
        print("❌ No checkpoint found!")
        print("   Searched in:")
        for path in possible_checkpoint_paths:
            exists = "✓" if os.path.exists(path) else "✗"
            print(f"   [{exists}] {os.path.abspath(path)}")
        print("\n⚠️  Model will use random weights (not trained)")
        print("   To train: cd ai_cybersec_custom/train && python3 train.py")

    model = model.to(device)
    model.eval()
    return model, tokenizer


def top_p_top_k_sampling(logits: torch.Tensor, top_p: float = 0.95, top_k: int = 40, temperature: float = 1.0) -> int:
    """Apply combined top-p and top-k sampling."""
    logits = logits / temperature

    # Top-k
    if top_k > 0:
        kth_vals = torch.topk(logits, top_k)[0][..., -1, None]
        indices_to_remove = logits < kth_vals
        logits = logits.masked_fill(indices_to_remove, float('-inf'))

    # Top-p (nucleus)
    sorted_logits, sorted_indices = torch.sort(logits, descending=True)
    cumulative_probs = torch.cumsum(torch.softmax(sorted_logits, dim=-1), dim=-1)

    sorted_indices_to_remove = cumulative_probs > top_p
    sorted_indices_to_remove[..., 0] = False
    sorted_logits = sorted_logits.masked_fill(sorted_indices_to_remove, float('-inf'))

    probs = torch.softmax(sorted_logits, dim=-1)
    next_idx = torch.multinomial(probs, num_samples=1)
    next_token = sorted_indices[next_idx].item()

    return next_token


def generate_response(model: torch.nn.Module, tokenizer: CustomTokenizer, prompt: str, device: str = 'cpu') -> str:
    """Generate response from prompt using the model."""
    max_length = INFER_CONFIG.get('max_response_length', 128)
    temperature = INFER_CONFIG.get('temperature', 1.0)
    top_p = INFER_CONFIG.get('top_p', 0.95)
    top_k = INFER_CONFIG.get('top_k', 40)

    formatted_prompt = f"User: {prompt}\nAgent:"

    ids = tokenizer.encode(formatted_prompt, add_bos=True)
    response_ids = ids.copy()

    with torch.no_grad():
        for step in range(max_length):
            input_tensor = torch.tensor([response_ids], dtype=torch.long, device=device)

            # Truncate if too long
            if input_tensor.size(1) > MODEL_CONFIG['seq_len']:
                input_tensor = input_tensor[:, -MODEL_CONFIG['seq_len']:]

            logits, _ = model(input_tensor)
            next_token = top_p_top_k_sampling(
                logits[0, -1],
                top_p=top_p,
                top_k=top_k,
                temperature=temperature
            )

            if next_token == tokenizer.EOS or next_token == tokenizer.PAD:
                break

            response_ids.append(int(next_token))

    generated = response_ids[len(ids):]
    if generated:
        try:
            response = tokenizer.decode(generated)
            response = response.strip()
            if "User:" in response:
                response = response.split("User:")[0].strip()
        except Exception as e:
            print(f"⚠️  Decode error: {e}")
            response = "(Error decoding response)"
    else:
        response = "(No response generated)"

    return response


def chat():
    """Interactive chat loop with the model."""
    print("\n" + "=" * 70)
    print("🤖 CUSTOM TRANSFORMER CHAT AGENT - CYBERSECURITY ASSISTANT")
    print("=" * 70)

    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    print(f"📍 Device: {device}")
    print("💬 Type 'exit' to quit, 'clear' to clear history")
    print("=" * 70 + "\n")

    try:
        model, tokenizer = load_model_and_tokenizer(device=device)
        # try to print parameter count if available
        try:
            params = sum(p.numel() for p in model.parameters())
            print(f"✅ Model ready | Parameters: {params:,}")
        except Exception:
            print("✅ Model ready")
        try:
            print(f"   Vocab size: {tokenizer.vocab_size()}")
            print(f"   Special tokens: PAD={tokenizer.PAD}, BOS={tokenizer.BOS}, EOS={tokenizer.EOS}\n")
        except Exception:
            pass
    except Exception as e:
        print(f"\n❌ Failed to load model: {e}")
        print("\nPlease ensure:")
        print("  1. Tokenizer trained: python ai_cybersec_custom/tokenizer/train_tokenizer.py")
        print("  2. Model trained: cd ai_cybersec_custom/train && python train.py")
        import traceback
        traceback.print_exc()
        return

    conversation_history = []

    while True:
        try:
            prompt = input("👤 You: ").strip()

            if prompt.lower() == 'exit':
                print("\n👋 Goodbye! Stay secure!")
                break

            if prompt.lower() == 'clear':
                conversation_history = []
                print("🗑️  Conversation history cleared\n")
                continue

            if not prompt:
                print("⚠️  Please enter a message\n")
                continue

            conversation_history.append(("user", prompt))

            print("🤔 Generating response...", end=" ", flush=True)
            response = generate_response(model, tokenizer, prompt, device=device)
            print("\r" + " " * 50 + "\r", end="")  # Clear "Generating..." line
            print(f"🤖 Agent: {response}\n")

            conversation_history.append(("agent", response))

        except KeyboardInterrupt:
            print("\n\n👋 Interrupted. Goodbye!")
            break
        except Exception as e:
            print(f"\n❌ Error: {e}")
            import traceback
            traceback.print_exc()
            print("Continuing...\n")


def main():
    """Entry point."""
    chat()


if __name__ == "__main__":
    main()
